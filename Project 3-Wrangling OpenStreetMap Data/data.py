...
def shape_element(element, node_attr_fields=NODE_FIELDS, way_attr_fields=WAY_FIELDS,
    ...
    if element.tag == 'node':
        ...
        for child in element:
            ...
            if child.attrib['k'] == 'telephone': # Update telephone number
                new_value = update_number(child.attrib['v'])
                dict['value'] = new_value
            if child.attrib['k'] == 'building':
                new_value = update_building(child.attrib['v'])
                dict['value'] = new_value
            tags.append(dict)
            # print tags
...
def update_number(number):  #update the telephone number to 1-888-888-8888 format
    if phone_is_valid(number)[0]:
         return phone_is_valid(number)[1]
